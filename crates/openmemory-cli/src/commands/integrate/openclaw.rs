//! `openmemory integrate openclaw` — register in OpenClaw's config.
//!
...
mod tests {
    ...
    fn creates_new_config_when_missing() {
        ...
        with_home(dir.path(), || {
            ...
            .unwrap();
            let entry = parsed
                .pointer("/mcp/servers/openmemory")
    ...
    fn preserves_sibling_servers() {
        ...
        with_home(dir.path(), || {
            ...
            assert!(parsed.pointer("/mcp/servers/openmemory").is_some());
    ...
    fn updates_existing_entry_then_idempotent() {
        ...
        write_file(
            &cfg,
            r#"{"mcp":{"servers":{"openmemory":{"command":"old-bin","args":["mcp"]}}}}"#,
        );
        with_home(dir.path(), || {
            std::env::set_var("OPENCLAW_CONFIG_PATH", &cfg);
            run("default", args(None)).unwrap();
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&cfg).unwrap()).unwrap();
            let entry = parsed.pointer("/mcp/servers/openmemory").unwrap();
            assert_eq!(entry["command"], "openmemory");
        });
    }
    ...
    fn non_default_profile_renames_entry() {
        ...
        with_home(dir.path(), || {
            ...
            .unwrap();
            assert!(parsed.pointer("/mcp/servers/openmemory-work").is_some());
            assert!(parsed.pointer("/mcp/servers/openmemory").is_none());
        });
    }
}
