//! `open-memory integrate claude-desktop` — register the MCP server in
...
pub fn run(profile: &str, args: IntegrateClaudeDesktopArgs) -> Result<()> {
    ...
    if matches!(outcome, IntegrationOutcome::Unchanged) {
        println!(
            "open-memory: claude-desktop config at {} already has matching `{name}` entry — no changes",
            config_path.display()
        );
        return Ok(());
    }
    ...
    println!(
        "open-memory: claude-desktop integration ready. Restart Claude Desktop to pick it up."
    );
    Ok(())
}
...
fn print_outcome(outcome: &IntegrationOutcome, name: &str, path: &Path) {
    match outcome {
        IntegrationOutcome::Created => {
            println!(
                "open-memory: created claude-desktop config at {}",
                path.display()
            );
        }
        IntegrationOutcome::Added => {
            println!(
                "open-memory: added `{name}` to {} (mcpServers)",
                path.display()
            );
        }
        IntegrationOutcome::Updated => {
            println!(
                "open-memory: updated `{name}` in {} (mcpServers)",
                path.display()
            );
        }
...
mod tests {
    ...
    fn creates_new_config_when_missing() {
        ...
        with_home(dir.path(), || {
            ...
            let entry = parsed
                .pointer("/mcpServers/open-memory")
    ...
    fn preserves_sibling_servers() {
        ...
        with_home(dir.path(), || {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&cfg).unwrap()).unwrap();
            assert!(parsed.pointer("/mcpServers/open-memory").is_some());
    ...
    fn non_default_profile_renames_entry() {
        ...
        with_home(dir.path(), || {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&cfg).unwrap()).unwrap();
            assert!(parsed.pointer("/mcpServers/open-memory-work").is_some());
            assert!(parsed.pointer("/mcpServers/open-memory").is_none());
        });
    }
}
