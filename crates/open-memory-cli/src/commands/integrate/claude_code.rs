//! `open-memory integrate claude-code` — register the MCP server in
...
pub fn run(profile: &str, args: IntegrateClaudeCodeArgs) -> Result<()> {
    ...
    if !args.no_cli && try_cli_add(&name, &entry)? {
        println!("open-memory: registered `{name}` via `claude mcp add-json`.");
        println!("open-memory: claude-code integration ready.");
        return Ok(());
    }
    ...
    if matches!(outcome, IntegrationOutcome::Unchanged) {
        println!(
            "open-memory: claude-code config at {} already has matching `{name}` entry — no changes",
            config_path.display()
        );
        return Ok(());
    }
...
fn print_outcome(outcome: &IntegrationOutcome, name: &str, path: &Path) {
    match outcome {
        IntegrationOutcome::Created => {
            println!(
                "open-memory: created claude-code config at {}",
                path.display()
            );
        }
...
mod tests {
    ...
    fn http_entry() {
        ...
        with_home(dir.path(), || {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&cfg).unwrap()).unwrap();
            let entry = parsed.pointer("/mcpServers/open-memory").unwrap();
