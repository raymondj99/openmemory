//! `open-memory consolidate` — run the dedup + decay-prune pipeline once.

use anyhow::{Context, Result};
use open_memory_core::config::Config;
use open_memory_graph::{ConsolidateConfig, MemoryStore};
...
pub fn run(profile: &str, args: ConsolidateArgs) -> Result<()> {
    ...
    println!("open-memory consolidate report");
...
mod tests {
    ...
    use open_memory_graph::ObservationInput;
    ...
    fn consolidate_dedups_real_data() {
        ...
        with_home(dir.path(), || {
            ...
            store
                .remember(
                    "X",
                    open_memory_graph::EntityType::Fact,
                    &[
