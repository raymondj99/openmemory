//! `open-memory watch <PATH>` — start a foreground filesystem
//! watcher.
//!
...
use open_memory_watch::{WatchOptions, Watcher, DEFAULT_EXTENSIONS};
...
pub fn run(profile: &str, args: WatchArgs) -> Result<()> {
    ...
    eprintln!("open-memory watch: watching {}", watcher.root().display());
    eprintln!(
        "open-memory watch: send SIGINT (Ctrl-C) or SIGTERM to stop; SQLite is in WAL mode \
    ...
    eprintln!(
        "open-memory watch: stopped — inserted={}, updated={}, removed={}, unchanged={}, skipped={}",
