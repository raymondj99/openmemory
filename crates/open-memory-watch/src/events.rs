...
//! `.open-memory-ignore` via [`ignore::WalkBuilder`]. Runtime events
...
fn handle_upsert(
    ...
    match process_file(memory, path, options) {
        ...
        Err(e) => warn!(
            target: "open_memory_watch::events",
            path = %path.display(),
            error = %e,
            "process_file failed"
        ),
...
/// Runtime events skip the full `.gitignore` / `.open-memory-ignore`
...
mod tests {
    ...
    use open_memory_index::SourceKind;
