...
//!
//! Seven tools wrap the [`open_memory_graph::MemoryStore`] API:
//!
//! - `open_memory_remember` — atomic write (entity + observations + relations)
//! - `open_memory_recall` — hybrid search + decay scoring
//! - `open_memory_list_entities` — paginated entity index
//! - `open_memory_get_entity` — entity + observations + relations bundle
//! - `open_memory_forget` — soft-delete a single observation
//! - `open_memory_forget_entity` — hard-delete an entity (cascade)
//! - `open_memory_status` — counts and timestamps
//!
...
pub struct RememberInput {
    /// Entity name (e.g., "Raymond", "open-memory project", "Rust").
    pub entity: String,
...
pub struct ForgetInput {
    /// Observation ID (UUIDv7) returned by `open_memory_recall` or
    /// `open_memory_remember`.
