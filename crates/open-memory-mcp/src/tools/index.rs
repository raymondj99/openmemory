...
//!
//! Three tools wrap the [`open_memory_index::engine::OpenEngine`] that the
//! [`open_memory_graph::MemoryStore`] holds:
//!
//! - `open_memory_index_text` — store arbitrary text under a caller-supplied
//!   URI for keyword-and-semantic recall via `open_memory_search`.
//! - `open_memory_search` — hybrid (vector + keyword) search across every
//!   indexed source.
//! - `open_memory_delete` — drop a URI from the index.
//!
...
use open_memory_index::traits::IndexEntry;
...
const INDEX_TEXT_DESC: &str =
    "Index a piece of text under a caller-supplied URI for hybrid (vector + keyword) recall \
     via open_memory_search. Use this for raw notes, snippets, or any content you want to \
