//! Tool registry for the openmemory MCP server.
//!
...
//! - [`maintenance`] — `openmemory_consolidate`
...
pub trait Tool: Send + Sync + 'static {
    /// Tool name as advertised over MCP (e.g. `"openmemory_remember"`).
...
mod tests {
    ...
    fn instructions_block_has_workflow_section() {
        let text = server_instructions();
        assert!(text.contains("openmemory is a local persistent agent memory"));
    ...
    fn router_lists_descriptors_in_registration_order() {
        ...
        assert_eq!(names[0], "openmemory_remember");
    }
}
