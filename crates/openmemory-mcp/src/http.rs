...
//! [`BEARER_TOKEN_ENV`] (`OPENMEMORY_HTTP_TOKEN`) before launching the
    ...
    if auth.is_some() {
        ...
        tracing::warn!(
            ...
            "HTTP transport: no bearer-token auth configured \
             ...
             set OPENMEMORY_HTTP_TOKEN to require auth",
        );
    }
...
mod tests {
    ...
    async fn initialize_round_trip() {
        ...
        assert!(body.contains("\"name\":\"openmemory\""));
    }
    ...
    async fn tools_list_round_trip() {
        ...
        .await;
        assert_eq!(status, StatusCode::OK);
        assert!(body.contains("openmemory_remember"));
    }
