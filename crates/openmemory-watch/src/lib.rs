//! Filesystem watcher with incremental re-indexing for `openmemory`.
//!
...
//! re-run of `openmemory watch` over an unchanged tree is free), then
...
//! future `openmemory mcp --watch DIR` can share the same handle the
...
/// Filesystem watcher. Constructed once per `openmemory watch` run.
///
...
mod tests {
    ...
    #[test]
    fn ignore_file_name_is_dot_openmemory_ignore() {
        assert_eq!(IGNORE_FILE_NAME, ".openmemory-ignore");
    }
}
