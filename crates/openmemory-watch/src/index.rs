...
//! makes a re-run of `openmemory watch` over an unchanged tree free).
...
//! convention used elsewhere in openmemory (`memory://observation/<id>`,
...
//! `openmemory_index_text` tool — free-text URI content) and tag the
...
use openmemory_index::{IndexEntry, SourceKind, SourceRecord};
    ...
    pub fn scan_initial(&self) -> WatchResult<ScanReport> {
        ...
        for path_result in crate::scan::iter_indexable(walker, &self.options) {
            let path: PathBuf = match path_result {
                Ok(p) => p,
                Err(e) => {
                    warn!(target: "openmemory_watch::scan", error = %e, "skip walker entry");
                    continue;
                }
            };
