//! Error types for `openmemory-watch`.
//!
...
pub enum WatchError {
    ...
    /// `ignore` failed to parse a `.gitignore` / `.openmemory-ignore`
    /// file or hit a permission error during the walk.
    #[error("ignore-walk error: {0}")]
    Ignore(#[from] ignore::Error),

    /// Memory store / index error from the underlying `openmemory-graph`
    /// crate.
    ...
    Index(#[from] openmemory_index::IndexError),
