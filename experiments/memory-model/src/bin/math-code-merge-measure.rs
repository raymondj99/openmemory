...
fn main() {
    black_box(evaluate_math_code_repository_merge().expect("warm-up evaluation succeeds"));
    let started = Instant::now();
    for _ in 0..ITERATIONS {
        black_box(evaluate_math_code_repository_merge().expect("measured evaluation succeeds"));
    }
