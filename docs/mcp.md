...
| Tool | Type | Purpose |
|------|------|---------|
| `openmemory_remember` | write | Create or update an entity, append observations and relations atomically. Fuzzy-matches incoming names against existing entities of the same type to prevent duplicates (configurable thresholds in `[normalization]`). Accepts an optional `memory_tier` field (defaults to `episodic`). Each entry in `observations` is either a bare string (legacy shape) or an object with `content` plus optional `title`, `summary`, `importance`, `source_kind`, `concepts`, `source_files`. With the write-behind context engine enabled (`[engine] enabled = true` in config.toml), the write is enqueued into a sharded, journaled ingestion lane and the response becomes a receipt `{accepted, durable, entity, shard, seq}`; the optional `durable` input (default: the configured `engine.durable_ack`) controls whether the call waits for the SQLite commit before returning. |
