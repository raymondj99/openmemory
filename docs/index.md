...
1. **SQLite is the storage layer.** Entities, observations,
   ...
   services. The CLI and MCP server keep working without a daemon;
   the optional local daemon is a loopback-only desktop/admin control
   plane.
...
| [crates.md](crates.md) | Per-crate reference: purpose, feature flags, public API surface, key types, and source-file map for each workspace crate, including the daemon/admin platform crates and benchmark/eval crates. |
...
| [cli.md](cli.md) | Reference for every `openmemory` subcommand and its flags: `init`, `status`, `daemon`, `mcp`, `consolidate`, `integrate openclaw`, `remember`, `recall`, `list-entities`, `forget-entity`, `completions`, `watch`. |
