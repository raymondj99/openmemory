...
Current release: **v0.3.0**. Production-hardening pass on top of
v0.2.0: entity normalization on the `remember` write path,
explicit embedding-model management (`openmemory model list /
download / use`), SHA-256 integrity verification for ONNX models,
optional bearer-token auth on the Streamable HTTP transport, and
three new CI gates. Multi-agent memory (a pool of read-only WAL
connections so concurrent recalls run in parallel) and the
`openmemory watch DIR` filesystem watcher landed in v0.2.0. The
MCP tool surface is unchanged at v0.1.
...
| [roadmap.md](roadmap.md) | What shipped in v0.1.0, v0.2.0, and v0.3.0, what is in flight on `[Unreleased]`, and the post-v0.3 backlog (Homebrew, LLM features, Postgres backend, etc.). |
